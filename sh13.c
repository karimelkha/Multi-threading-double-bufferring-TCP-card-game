// include des interfaces des bibliotheques
#include <SDL.h>        
#include <SDL_image.h>        
#include <SDL_ttf.h>        
#include <pthread.h>
...
pthread_t thread_serveur_tcp_id; // thread
...
void *fn_serveur_tcp(void *arg) // fonction thread pour qu'il marche
{
...
void sendMessageToServer(char *ipAddress, int portno, char *mess) // envoyer un message au serveur
...
int main(int argc, char ** argv)
{
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		//printf("un event\n");
			...
			case  SDL_MOUSEBUTTONDOWN:
				...
				if ((mx<200) && (my<50) && (connectEnabled==1))
				{
					...
					// envoyer un message à destination du serveur principal pour lui dire
					// que je suis machin et je me connecte
				...
				else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
				{
					...
					if (guiltSel!=-1)
					{
						...
						sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
					// RAJOUTER DU CODE ICI
					// envoyer du code a destination du serveur 
        ...
        if (synchro==1) // si synchro=1, alors le serveur reseau a recu un message et il fuat le consulter
        {
                printf("consomme |%s|\n",gbuffer);
		switch (gbuffer[0])
		{
			...
			case 'I':
				// RAJOUTER DU CODE ICI
				// mettre a jour identifiant avec la valeur
				
				printf("J'ai recu mon id : %c\n", gbuffer[2]);
				gId = digit_to_int(gbuffer[2]);
				strcpy(gNames[0],gName);
				break;
			// Message 'L' : le joueur recoit la liste des joueurs
			case 'L':
				sscanf(gbuffer, "%c %s %s %s %s",&commande,gNames[0],gNames[1],gNames[2],gNames[3]);
			
				break;
			// Message 'D' : le joueur recoit ses trois cartes
			case 'D':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer, "%c %d %d %d %d %d %d %d %d %d %d %d",&commande,&b[0],&b[1],&b[2],&tableCartes[gId][0],&tableCartes[gId][1],&tableCartes[gId][2],&tableCartes[gId][3],&tableCartes[gId][4],&tableCartes[gId][5],&tableCartes[gId][6],&tableCartes[gId][7]);
				break;
			// Message 'M' : le joueur recoit le n° du joueur courant
			// Cela permet d'affecter goEnabled pour autoriser l'affichage du bouton go
			case 'M':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer, "%c %d",&commande,&id);
				if(gId==id){
					goEnabled=1;
				}else{
					goEnabled=0;
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer, "%c %d %d %d ",&commande,&temp,&ligne,&colonne);
				tableCartes[ligne][colonne] = temp;
				break;
		}
		synchro=0;
        }
	...
	// pour afficher les trois cartes
	if (b[0]!=-1) 
	{
